...
impl LsmStorageInner {
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<Bytes>> {
        ...
        let result = match value {
            Some(tombstone) if tombstone.is_empty() => None,
            Some(v) => Some(v),
            None => None,
        };
        Ok(result)
    ...
    pub fn force_freeze_memtable(&self, _state_lock_observer: &MutexGuard<'_, ()>) -> Result<()> {
        let memtable = Arc::new(MemTable::create(self.next_sst_id()));
        {
            ...
            let freezed_memtable = std::mem::replace(&mut guard.memtable, memtable);
            guard.imm_memtables.insert(0, freezed_memtable);
            *state = Arc::new(guard);
        }
        Ok(())
    }
